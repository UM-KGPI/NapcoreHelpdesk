...
- Note that the current product now separates user chat and operator console into independent routes.
...
- Describe `/user` as the primary conversation surface.
- Mention `/operator` as the separate governance and board surface.
...
- Mention that the route switcher now separates `/user` and `/operator` for demo clarity.
